      ...
      async function handleSubmit(e) {
        ...
        const id = toast.loading("Please wait...", { position: "top-center", autoClose: true, toastId: customId })
         try {
            ...
            if (form.message.length < 5) return toast.update(id,{ render:"The message must be at least 5 characters!", type:"error",isLoading:false, position: "top-center", autoClose: true, toastId: customId});
            const response = await axios.post(`${api}/misc/contact-us`, form)
                if (response.data.success === true) {
                    toast.update(id, { render: "Message sent successfully", type: "success", isLoading:false, position: "top-center" , autoClose: true, toastId: customId});
                ...
                if (error.response.status === 500) {
                    ...
                    toast.update(id, { render: error.response.data.error.message, type:"error", position:"top-center", isLoading:false, autoClose:true, toastId: customId });
                   }

                
                else if (error.request ) {
                    toast.update(id, { render:"You are offline", type:"error", isLoading:false , position:"top-center", autoClose:true, toastId: customId });
               }
                ...
                <div class="grid bg-white rounded-lg w-11/12 md:w-full lg:w-full sm:mx-6 sm:ml-20">
                    <form onSubmit={handleSubmit}>
                        <div class="grid grid-cols-1 gap-5 sm:gap-y-8 mx-2">
                            ...
                            <div class="grid grid-cols-1">
                                <label className='sm:text-2xl text-md font-Gilroy font-semibold text-gray-500' htmlFor="message">Message</label>
                                    <textarea
                            ...
                            <div className='text-center mt-5 sm:mt-10'>
                                <button className='bg-black w-auto font-Gilroy text-white px-5 sm:px-9 sm:py-4 py-3 rounded-md font-semibold border-none active:bg-gray-600' type="submit">Submit</button>
                            </div>
                    </form>
                </div>         
            </div>
